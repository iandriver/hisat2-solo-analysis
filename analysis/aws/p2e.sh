#!/bin/bash
# Phase 2e -- the graph index, 64-bit, SNVs only.
#
...
#
# Dropping the indels was not enough: 12.64M SNVs still exceeded 2^32 nodes.
# The binding quantity is haplotypes, not variants. Real phasing puts a variant
# into several observed combinations, so this set carries 14.0M haplotypes for
# 12.6M variants (1.11 each), against roughly 0.78 for the greedy-colouring
# partition the shipped index was built from -- about 46% more distinct paths
# through the graph. Cutting 15% of the variants moved the haplotype count only
# 14%, which is why it did not help.
#
# So: 64-bit, keeping real phasing. The previous 64-bit attempt was OOM-killed
# at 368.8 GB with 14.95M variants / 16.34M haplotypes; this set is ~15% smaller
# on both, predicting a peak near 312 GB against the 371 GB available.
set -eux
set -o pipefail
export HOME=/root
...
cd $D
[ -s genome.snv.snp ] || { aws s3 cp "$BUCKET/hisat2-p2e/refs/genome.snv.snp.gz" - | pigz -dc > genome.snv.snp; }
[ -s genome.snv.haplotype ] || { aws s3 cp "$BUCKET/hisat2-p2e/refs/genome.snv.haplotype.gz" - | pigz -dc > genome.snv.haplotype; }
[ -s $D/genome.fa ]
...
rm -rf $D/idx_snv $D/idx_snv_l; mkdir -p $D/idx_snv_l
trace build_snv hisat2-build-l -p "$(nproc)" \
  --snp $D/genome.snv.snp --haplotype $D/genome.snv.haplotype \
  $D/genome.fa $D/idx_snv_l/genome
RET=$(grep -o 'Local indexes: .*' $OUT/build_snv.log | tail -1)
SZ=$(du -sk $D/idx_snv_l | cut -f1)
...
ls $D/idx_snv_l/genome.1.ht2l >/dev/null 2>&1 || core_ok=0
note "GATE core_ok=$core_ok"

if [ "$core_ok" -eq 1 ]; then
  aws s3 sync $D/idx_snv_l "$BUCKET/$RUN/refs/grch38_v50_1kgp_snv_large/" --quiet
  note "INDEX_UPLOADED bytes=$(du -sb $D/idx_snv_l | cut -f1)"
fi
